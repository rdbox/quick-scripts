...
while (1) {
    ...
    if ($@) {
        ...
        # timed out
		print "Vulnerable - Remote host is affected\n";
		print $fh "$host:$port : VULNERABLEf \n";
		last;
    }

	if (!defined($ret)) {
		print STDERR "sysread error: $!\n";
		print  $fh "$host:$port : sysreaderror \n";
	}
	...
	if ($ret > 0) {
		if ($ret == 2) {
			...
			print  $fh "$host:$port : SAFE \n";
			$rv = 1;
		} elsif ($ret == 1) {
			print "ERROR Remote host is probably not affected\n";
			print  $fh "$host:$port : Maybe SAFE \n";
		}
		last;
	}
